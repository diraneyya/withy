#!/usr/bin/env bash
#
# install.sh — install Whisperbar on macOS.
#
...
APP="$HOME/.local/share/whisperbar/app"
BIN="$HOME/.local/bin/whisperbar"
CONF="$HOME/.config/whisperbar"
SPOONS="$HOME/.hammerspoon/Spoons"
INIT="$HOME/.hammerspoon/init.lua"
LOADLINE='hs.loadSpoon("Whisperbar"):start()'

say()  { printf '\033[1m==>\033[0m %s\n' "$*"; }
warn() { printf '\033[33m  ! %s\033[0m\n' "$*"; }

[[ "$(uname -s)" == "Darwin" ]] || { echo "Whisperbar is macOS-only." >&2; exit 1; }
...
# ── application ──────────────────────────────────────────────────────────
say "Installing Whisperbar"
mkdir -p "$APP" "$(dirname "$BIN")" "$CONF" "$SPOONS"
rm -rf "${APP:?}/whisperbar"
cp -R "$SRC/whisperbar" "$APP/whisperbar"
...
exec /usr/bin/python3 -m whisperbar "\$@"
EOF
chmod +x "$BIN"

rm -rf "$SPOONS/Whisperbar.spoon"
cp -R "$SRC/Whisperbar.spoon" "$SPOONS/Whisperbar.spoon"
...
if [[ -f "$INIT" ]] && grep -qF 'loadSpoon("Whisperbar")' "$INIT"; then
  echo "  hammerspoon already configured"
else
  say "Adding Whisperbar to your Hammerspoon config"
  [[ -f "$INIT" ]] && cp "$INIT" "$INIT.whisperbar-backup.$(date +%s)"
  printf '\n-- Whisperbar (local dictation)\n%s\n' "$LOADLINE" >> "$INIT"
fi
